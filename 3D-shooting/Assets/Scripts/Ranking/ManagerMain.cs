...
public class ManagerMain : MonoBehaviour
{
    ...
    /// Raises the click get json from www event.
    /// </summary>
    public void OnClickSetRankingApi()
    {
        displayField.text = "wait...";
        SetJsonFromWww();
    }
    ...
    {
        // APIが設置してあるURLパス
        const string url = "http://localhost/shootingrankingsystemAPI/shootingranking/getRankings";
    ...
    private void CallbackWwwSuccess(string response)
    {
        ...
        List<RankingData> rankingList = RankingDataModel.DeserializeFromJson(response);

        string sStrOutput = "";
        foreach (RankingData ranking in rankingList)
        {
            sStrOutput += $"Name:{ranking.Name}";
            sStrOutput += $"　";
            sStrOutput += $"Score:{ranking.Score}\n";
        }
    ...
    private void SetJsonFromWww()
    {
        ...
        // APIが設置してあるURLパス
        string sTgtURL = "http://localhost/shootingrankingsystemAPI/shootingranking/setRanking";
